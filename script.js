...
let appSettings = { minDeposit: 100, minWithdraw: 500 };
...
const loadAppSettings = async () => {
  try {
    ...
    const depMinText = document.getElementById('dep-min-text');
    if (depMinText) depMinText.textContent = `Min: ৳${appSettings.minDeposit}`;

    const wdMinText = document.getElementById('wd-min-text');
    if (wdMinText) wdMinText.textContent = `Minimum withdrawal: ৳${appSettings.minWithdraw} | Processing time: 1-24 hours`;
...
async function loadDashboardData() {
  try {
    ...
    renderLevelBreakdown(refData.tree || []);
...
function copyRef() {
  ...
  navigator.clipboard.writeText(link)
    .then(() => showToast('Referral ID copied! 🔗', 'success'))
    .catch(() => {
...
async function handleDeposit(e) {
  ...
  if (parseFloat(amount) < appSettings.minDeposit) {
    showToast(`Minimum deposit is ৳${appSettings.minDeposit}`, 'error');
    return;
  }
  showLoader();
  try {
...
async function handleWithdraw(e) {
  ...
  if (parseFloat(amount) < appSettings.minWithdraw) {
    showToast(`Minimum withdrawal is ৳${appSettings.minWithdraw}`, 'error');
    return;
  }
...
async function handleSpin() {
  ...
  try {
    ...
    const prizeAngle = 360 - (prizeIndex * 60) - 30;
